...
bool validate_op_addr(line_info line, addressing_type op1_addressing, addressing_type op2_addressing, int op1_valid_addr_count, int op2_valid_addr_count, ...);
...
code_word *get_code_word(line_info line, opcode curr_opcode, int op_count, char *operands[2]) {
	...
	/* Set the ARE bits to default 00 (2 bits) */
	codeword->ARE = 0;
	/* Clear the upper 6 bits of word */
	codeword->word = 0;
	/* Set the opcode bits (5 to 8) */
	codeword->word |= (curr_opcode << 5);

	/* Check if need to set the addressing bits */
	if (curr_opcode >= MOV_OP && curr_opcode <= LEA_OP) { /* First Group, two operands */
		/* Set the source addressing bits (0 to 1) */
		codeword->word |= (first_addressing & 0x3);
		/* Set the destination addressing bits (2 to 4) */
		codeword->word |= ((second_addressing & 0x7) << 2);
	} else if (curr_opcode >= CLR_OP && curr_opcode <= PRN_OP) {
		/* Set the destination addressing bits (2 to 4) */
		codeword->word |= ((first_addressing & 0x7) << 2);
	}
	return codeword;
}
bool validate_op_addr(line_info line, addressing_type op1_addressing, addressing_type op2_addressing, int op1_valid_addr_count,
 ...
 data_word *build_data_word(addressing_type addressing, long data, bool is_extern_symbol) {
	...
	dataword->word = 0;
	/* Set the ARE bits (0 to 1) */
	if (addressing == DIRECT_ADDR) {
		dataword->ARE = is_extern_symbol ? 1 : 2;
	} else {
		dataword->ARE = 4;
	}

	/* Set the data bits (2 to 11) */
	dataword->word = data & 0x3FF;
