...
bool find_label(line_info line, char *symbol_dest) {
	...
	if (line.content[i] == ':') {
		if (!is_valid_label_name(symbol_dest)) {
			printf_line_error(li,"Invalid label name - cannot be longer than 32 chars, may only start with letter be alphanumeric.");
			symbol_dest[0] = '\0';
...
int printf_line_error(line_info line, char *message, ...) { /* Prints the errors into a file, defined above as macro */
	...
	result = fprintf(ERR_OUTPUT_FILE, message, args);
	va_end(args);
