...
#define MOVE_TO_NOT_WHITE(string, index)   for (;string[(index)] && (string[(index)] == '\t' || string[(index)] == ' '); (++(index)));
        
bool is_int(char *string) ;

int get_register_by_name( char *name);
/** moves the index to the next placint get_register_by_name(const char *name);e in string where the char isn't white */
